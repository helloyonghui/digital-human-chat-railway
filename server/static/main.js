...
(function () {
    ...
    if (!LiveKit || typeof LiveKit.Room !== 'function') {
        throw new Error('LiveKit 客户端未正确加载（要求 window.LivekitClient）');
    }
    // 新增：防止模板选择器重复初始化（云端页面可能触发两次）
    let TPL_INIT_DONE = false;
    ...
    function handleCommandResponse(response) {
        try {
            console.log('[LiveKit] Processing command response:', response);
            
            const responseType = response.response_type || response.type;
            const data = response.data;
            
            // 分发响应事件给相应的模块
            const event = new CustomEvent('livekit-response', {
                detail: {
                    type: responseType,
                    data: data,
            ...
            // 处理特定响应类型
            if (responseType === 'templates.list') {
                console.log('[LiveKit] Templates list response received');
            } else if (responseType === 'template.select') {
                if (response.success !== false) {
                    console.log('[LiveKit] Template selection confirmed:', response);
                } else {
                    console.warn('[LiveKit] Template selection failed:', response);
                    alert('模板选择失败: ' + (response.error || '未知错误'));
                }
            } else if (responseType === 'template_select_success') {
                console.log('[LiveKit] Template selection successful:', response);
            } else if (responseType === 'template_select_error') {
                console.warn('[LiveKit] Template selection error:', response);
                alert('模板选择失败: ' + (response.error || '未知错误'));
            } else if (responseType === 'server_ready') {
                console.log('[LiveKit] Server ready signal received');
            } else {
                console.log('[LiveKit] Unknown response type:', responseType);
            }
            
            console.log('[LiveKit] Command response received:', responseType, data);
        } catch (e) {
            console.warn('[LiveKit] Failed to handle command response:', e);
        }
    }
    ...
    async function ensureVideoPlayingWithSound() {
        try {
            ...
            if (audioEl) { applySpeakerGate(); await audioEl.play(); }
            startRenderer();
        } catch (e) {
    ...
    // 顶层工具函数：applySpeakerGate（在 ensureMicPermission 之后插入）
    ...
    // 通过LiveKit发送命令的全局函数
    window.sendCommandViaLiveKit = async function(command) {
        if (!window._lkRoom || window._lkRoom.state !== 'connected') {
            console.warn('[LiveKit] Room not connected, cannot send command:', command);
            throw new Error('LiveKit not connected');
        }
        
        try {
            const commandJson = JSON.stringify(command);
            await window._lkRoom.localParticipant.publishData(
                new TextEncoder().encode(commandJson)
            );
            console.log('[LiveKit] Command sent:', command);
        } catch (e) {
            console.error('[LiveKit] Failed to send command:', e);
            throw e;
        }
    };

    // ---- LiveKit-only：连接与发布 ----
    async function startCallLiveKitOnly(initialTemplateName){
        try {
            showCenterPrompt('正在连接...', { showSpinner:true });
            console.log('[LiveKit] Starting connection with template:', initialTemplateName);
            
            // 通过cloud_gateway获取LiveKit连接token
            const resp = await fetch('/lk/join', { method: 'GET' });
            
            if (!resp.ok) {
                const errorText = await resp.text();
                console.error('[LiveKit] HTTP error:', resp.status, errorText);
                throw new Error(`HTTP ${resp.status}: ${errorText}`);
            }
            
            const data = await resp.json();
            console.log('[LiveKit] Join response:', data);
            
            if (!data || !data.url || !data.token) {
                console.error('[LiveKit] Invalid response data:', data);
                throw new Error('lk/join 返回无效数据');
            }

            // 记录待发送的模板选择命令
            if (initialTemplateName) {
                window._pendingTemplateSelect = initialTemplateName;
            }
            ...
            room.on(LiveKit.RoomEvent.TrackSubscribed, (track,publication,participant) => {
                try {
                    ...
                    if (track.kind === LiveKit.Track.Kind.Video) {
                        ...
                        let gateShown = false;
                        // const expectedProduct = 960 * 540; // 定义期望的分辨率乘积
                        const expectedProduct = 1280 * 720; // 定义期望的分辨率乘积
            ...
            room.on(LiveKit.RoomEvent.DataReceived, (pkt) => {
                try {
                    const raw = new TextDecoder().decode(pkt);
                    console.log('[LiveKit] Raw data received:', raw);
                    if (!raw) return;
                    let msg = null;
                    try { msg = JSON.parse(raw); } catch(e) { 
                        console.warn('[LiveKit] Failed to parse JSON:', e, 'Raw:', raw);
                        msg = null; 
                    }
                    console.log('[LiveKit] Parsed message:', msg);
                    if (!msg || typeof msg !== 'object' || !('message_type' in msg)) {
                        // 检查是否为命令响应
                        if (msg && typeof msg === 'object' && msg.response_type) {
                            console.log('[LiveKit] Handling command response:', msg);
                            handleCommandResponse(msg);
                            return;
                        }
                        // 检查是否为其他类型的响应（如template_select_success等）
                        if (msg && typeof msg === 'object' && msg.type) {
                            console.log('[LiveKit] Handling typed response:', msg);
                            handleCommandResponse(msg);
                            return;
                        }
                        console.log('[LiveKit] Ignoring non-standard message:', msg);
                        return;
                    }
            ...
            room.on(LiveKit.RoomEvent.ConnectionStateChanged, async (st) => {
                if (st === LiveKit.ConnectionState.Connected) {
                    ...
                    if (audioEl) {  applySpeakerGate(); audioEl.play().catch(()=>{}); } } catch(e){}
                    updateOverlayPositions();
            ...
            window.sendCommandViaLiveKit = async function(payloadObj){
                try {
                    const bytes = new TextEncoder().encode(JSON.stringify(payloadObj||{}));
                    await room.localParticipant.publishData(bytes, { reliable:true });
                    console.log('[LiveKit] Command sent successfully:', payloadObj);
                } catch(e) { 
                    console.warn('publishData 命令发送失败', e); 
                    throw e;
                }
            };
            ...
            // 若存在待发送模板选择，连接后发送
            try {
                const pendingTpl = window._pendingTemplateSelect;
                if (pendingTpl) {
                    await window.sendCommandViaLiveKit({ cmd: 'template.select', template_name: String(pendingTpl) });
                    window._pendingTemplateSelect = null;
                }
            } catch (e) {
                console.warn('模板选择命令连接后发送失败：', e);
            }

            // 连接后：尝试请求麦克风权限并自动开启（但不强制要求成功）
            ...
            try {
                ...
                if (ok) {
                    try {
                        ...
                        console.warn('麦克风开启失败，但连接保持:', e);
                        // 不显示错误提示，避免影响用户体验
                        updateListenStatus('聆听中（麦克风未开启）');
                    }
                } else {
                    console.log('麦克风权限未获取，用户可稍后手动开启');
                    updateListenStatus('聆听中（麦克风未开启）');
                }
            } catch (e) {
                console.warn('麦克风初始化过程出错，但连接保持:', e);
                updateListenStatus('聆听中（麦克风未开启）');
            }
            ...
            showCenterPrompt(`连接失败: ${e.message}`, { variant:'danger', showSpinner:false });
            throw e;
        }
        }
        ...
        // 临时禁用页面生命周期停止逻辑，用于调试连接断开问题
        /*
        (function setupLifecycleStop(){
            let stopSent = false;
            async function sendStop(){
                // 只有在真正连接状态下才发送停止请求
                if (stopSent || state !== SessionState.CONNECTED) return;
                stopSent = true;
                console.log('[Lifecycle] Sending stop request due to page lifecycle event');
                try {
        ...
        */
        ...
        (function(){
            ...
            updateOverlayPositions();
            if (window.TemplateSelector && !TPL_INIT_DONE) {
                window.TemplateSelector.init({
                    onSelect: async (tplName) => {
                        try {
                            // 先记录待发送模板；房间未连时在连接后由 DataChannel 发送
                            window._pendingTemplateSelect = String(tplName);
                            state = SessionState.CONNECTING;
                            callBtn && (callBtn.disabled = true);
                            showCenterPrompt('正在连接...', { showSpinner:true });
                            // 云端：请求 /lk/join 获取 token（不依赖私网路由）
                            await startCallLiveKitOnly(String(tplName));
                            callBtn && (callBtn.disabled = false);
                            updateOverlayPositions();
                        } catch (e) {
                            state = SessionState.ERROR;
                            callBtn && (callBtn.disabled = false);
    ...
    // 防重复：如存在第二处模板选择初始化，统一加开关避免再次绑定
    if (window.TemplateSelector && !TPL_INIT_DONE) {
        window.TemplateSelector.init({
            onSelect: async (name) => {
                try {
                    window._pendingTemplateSelect = String(name);
                    // 已连接：走 DataChannel 指令
                    if (window._lkRoom && typeof window.sendCommandViaLiveKit === 'function' && state === SessionState.CONNECTED) {
                        await window.sendCommandViaLiveKit({ cmd: 'template.select', template_name: String(name) });
                        window._pendingTemplateSelect = null;
                    ...
                    state = SessionState.CONNECTING;
                    callBtn && (callBtn.disabled = true);
                    await startCallLiveKitOnly(String(name));
                    callBtn && (callBtn.disabled = false);
                    updateOverlayPositions();
                } catch (e) {
                    state = SessionState.ERROR;
